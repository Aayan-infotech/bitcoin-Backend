...
const userSignup = async (req, res, next) => {
  try {
    const { name, email, password, mobileNumber, gender, accountType } = req.body;
    ...
    const otp = "1111"; // Change in production to random OTP
...
const updateBiometric = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: `Biometric authentication ${user.biometricAuth ? "Enabled" : "Disabled"}`,
    });
  } catch (err) {
